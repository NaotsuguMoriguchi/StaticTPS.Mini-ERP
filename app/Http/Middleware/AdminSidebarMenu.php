                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
                            $sub->url(
                                ...
                                __('sale.add_sale'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'sells' && request()->segment(2) == 'create']
                            );
                        }
            ...
            if (in_array('stock_transfers', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create'))) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                         ...
                         if (auth()->user()->can('purchase.create')) {
                            $sub->url(
                                action('StockTempController@index'),
                                __('Add Stock Transfer'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'stock-transfers' && request()->segment(2) == 'create_temp']
                            );
                        }
                        ...
                        if (auth()->user()->can('purchase.view')) {
                            $sub->url(
                                action('StockTempController@prepare_transfer'),
                                __('Prepare Transfer'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'stock-transfers' && request()->segment(2) == null]
                            );
                        }
                        if (auth()->user()->can('purchase.create')) {
                            $sub->url(
                                action('StockTempController@receive_transfer'),
                                __('Receive Transfer'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'stock-transfers' && request()->segment(2) == 'create']
                            );
                        }
