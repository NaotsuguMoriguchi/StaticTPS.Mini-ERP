...
class ProductUtil extends Util
{
     ...
     * @return object
     */
    public function filterProductBy($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false){
        $query = Product::join('variations', 'products.id', '=', 'variations.product_id')
            ->active()
            ->whereNull('variations.deleted_at')
            ->leftjoin('units as U', 'products.unit_id', '=', 'U.id')
    ...
    public function filterTransferProduct($business_id, $search_term, $location_id = null, $transfer_location_id = null, $category_id = null, $sub_category_id = null, $brand_id = null, $search_fields) {
        $query = Product::join('variations', 'products.id', '=', 'variations.product_id')
            ->active()
            ->whereNull('variations.deleted_at')
            ->leftjoin('units as U', 'products.unit_id', '=', 'U.id')
            ->leftjoin('categories as CA', function ($join) {
                $join->on('products.category_id', '=', 'CA.id')
                    ->whereNull('CA.deleted_at');
            })
            ->leftjoin('categories as SCA', function ($join) {
                $join->on('products.sub_category_id', '=', 'SCA.id')
                    ->whereNull('SCA.deleted_at');
            })
            ->leftjoin('brands as BRD', function ($join) {
                $join->on('products.brand_id', '=', 'BRD.id')
                    ->whereNull('BRD.deleted_at');
            })
            ->leftjoin(
                'variation_location_details AS FVLD',
                function ($join) use ($location_id) {
                    $join->on('variations.id', '=', 'FVLD.variation_id');

                    //Include Location
                    if (!empty($location_id)) {
                        $join->where(function ($query) use ($location_id) {
                            $query->where('FVLD.location_id', '=', $location_id);
                        });
                        ;
                    }
                }
            )->leftjoin(
                'variation_location_details AS TVLD',
                function ($join) use ($transfer_location_id) {
                    $join->on('variations.id', '=', 'TVLD.variation_id');

                    //Include Location
                    if (!empty($transfer_location_id)) {
                        $join->where(function ($query) use ($transfer_location_id) {
                            $query->where('TVLD.location_id', '=', $transfer_location_id);
                        });
                        ;
                    }
                }
        ...
        //Include check for quantity
        $query->where('FVLD.qty_available', '>', 0);
        $query->where('TVLD.qty_available', '>=', 0);
        ...
        if (!empty($transfer_location_id)) {
            $query->ForLocation($transfer_location_id);
        }
        ...
        $query->select(
            ...
            'variations.id as variation_id',
            'variations.name as variation',
            'FVLD.qty_available as f_qty',
            'TVLD.qty_available as t_qty',
            'variations.sell_price_inc_tax as selling_price',
            'variations.sub_sku',
            'U.short_name as unit',
            'CA.name as category_name',
            'SCA.name as sub_category_name',
        ...
        $query->groupBy('variations.id');
        return $query->orderBy('FVLD.qty_available', 'desc')->get();
    }

    public function getTransferDetailsFromVariation($variation_id, $business_id, $location_id = null, $transfer_location_id = null)
    {
        $query = Variation::join('products AS p', 'variations.product_id', '=', 'p.id')
            ->join('product_variations AS pv', 'variations.product_variation_id', '=', 'pv.id')
            ->leftjoin('variation_location_details AS fvld', 'variations.id', '=', 'fvld.variation_id')
            ->leftjoin('variation_location_details AS tvld', 'variations.id', '=', 'tvld.variation_id')
            ->leftjoin('units', 'p.unit_id', '=', 'units.id')
            ->leftjoin('categories as cat', function ($join) {
                $join->on('p.category_id', '=', 'cat.id')
                    ->whereNull('cat.deleted_at');
            })
            ->leftjoin('categories as scat', function ($join) {
                $join->on('p.sub_category_id', '=', 'scat.id')
                    ->whereNull('scat.deleted_at');
            })
            ->leftjoin('brands', function ($join) {
                $join->on('p.brand_id', '=', 'brands.id')
                    ->whereNull('brands.deleted_at');
        ...
        $query->where(function ($query) use ($location_id) {
            $query->where('p.enable_stock', '!=', 1)
                ->orWhere('fvld.qty_available', '>', 0);
        });
        if (!empty($location_id)) {
            //Check for enable stock, if enabled check for location id.
            $query->where(function ($query) use ($location_id) {
                $query->where('p.enable_stock', '!=', 1)
                    ->orWhere('fvld.location_id', $location_id);
            });
        }

        $query->where(function ($query) use ($transfer_location_id) {
            $query->where('p.enable_stock', '!=', 1)
                ->orWhere('tvld.qty_available', '>=', 0);
        });
        if (!empty($transfer_location_id)) {
            //Check for enable stock, if enabled check for location id.
            $query->where(function ($query) use ($transfer_location_id) {
                $query->where('p.enable_stock', '!=', 1)
                    ->orWhere('tvld.location_id', $transfer_location_id);
            });
        }
