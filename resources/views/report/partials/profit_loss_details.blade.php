...
<div class="col-xs-12">
    @component('components.widget')
        ...
        <small class="help-block">(@lang('report.closing_stock') + @lang('home.total_sell') + @lang('lang_v1.total_sell_shipping_charge') + @lang('report.total_stock_recovered') + @lang('lang_v1.total_purchase_return') + @lang('lang_v1.total_purchase_discount') + @lang('lang_v1.total_sell_round_off') 
        @foreach($data['right_side_module_data'] as $module_data)
            @if(!empty($module_data['add_to_net_profit']))
                + {{$module_data['label']}} 
            @endif
        @endforeach
        ) <br> - (@lang('report.opening_stock') + @lang('home.total_purchase') + @lang('report.total_expense') + @lang('lang_v1.total_purchase_shipping_charge') + @lang('lang_v1.total_transfer_shipping_charge') + @lang('lang_v1.total_sell_discount') + @lang('lang_v1.total_reward_amount') 
        @foreach($data['left_side_module_data'] as $module_data)
            @if(!empty($module_data['add_to_net_profit']))
                + {{$module_data['label']}}
            @endif 
